function timeCourseMasked()
    ...
    for S = 1:length(params.subjects)
        ...
        for runNumber = [1:4]
            ...
            cmd = sprintf("fslmaths %s -mul %s %s", pscFileName, params.bilateralAuditoryCortexMask.path, maskedPscFileName)
            system(cmd);
            ...
            for t = 1:length(endTimes)
                ...
                currTrialData = pscMatrix(bilateralLinearIndex);
                currTrialData = currTrialData(startTimes(t):endTimes(t));

                % Each trial's time course is a column
                trialData(:, t) = currTrialData;
            end
            % Average time courses of all trials
            LHTrialData = mean(trialData(:, LHtrials(1:currMinTrials)), 2);
            RHTrialData = mean(trialData(:, RHtrials(1:currMinTrials)), 2);
