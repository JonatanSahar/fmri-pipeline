...
for s=params.subjects
    ...
    for session = 1:num_of_sessions
        scans=dir(fullfile(params.rawDCM,num2str(s),['session',num2str(session)]));
        scans={scans(find([scans.isdir])).name};
        scans(ismember(scans,{'.','..','ignore'}))=[];
        try
            load(fullfile(params.rawBehavioral,num2str(s),['trialOrder_Session',num2str(session),'.mat']));
        ...
        anatomy = dir(fullfile(params.rawDCM,num2str(s),['session',num2str(session)],'*MPRAGE*'));
        ...
        if ~exist(fullfile('..',params.expName,num2str(s),['session',num2str(session)],params.anatomyFolder,[num2str(s),'anatomy.nii.gz'])) || params.override
            file_dir = fullfile('..',params.expName,num2str(s),['session',num2str(session)],params.anatomyFolder);
            mkdir(fullfile(file_dir,'temp'));
            cmd = ['dcm2nii -o ' , fullfile(file_dir,'temp'),' ' fullfile(params.rawDCM,num2str(s),['session',num2str(session)],anatomy_file)];
            system(cmd);
        ...
        for i=1:length(scans)
            ...
            if strcmp(scan{2},'cmrr') && length(scan)==5
                ...
                file_dir = fullfile('..',params.expName,num2str(s),['session',num2str(session)], params.functionalFolder,params.conditions{session}{order(runNum)});
                if ~exist(fullfile(file_dir,file_name),'file') || params.override
                    mkdir(fullfile(file_dir,'temp'));
                    cmd = ['dcm2nii -o ' , fullfile(file_dir,'temp'),' ' fullfile(params.rawDCM,num2str(s),['session',num2str(session)],scans{i})];
                    system( cmd );
                    image = dir(fullfile(file_dir,'temp', '*.nii.gz'));
